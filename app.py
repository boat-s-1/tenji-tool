...
# --- 共通の保存用スクリプト（安定版） ---
# html2canvasの読み込みを確実にするため、ボタンと分離して定義します
download_logic = f"""
...
<div class="wrapper">
    ...
    <div class="main">
        <div class="left">
            <div class="mainbox" style="position: relative;">{stamp_html}<div class="section-title">本命候補</div><div style="font-size:40px; font-weight:bold; color:#ff4f93; margin-left:10px;">{honmei}</div><div class="score-row"><div style="font-size:24px; font-weight:bold;">イン逃げ期待度</div><div style="font-size:40px; font-weight:bold; color:#ff4f93;">{nige_rate}%</div></div><div class="score-row"><div style="font-size:24px; font-weight:bold;">場平均との差</div><div style="font-size:36px; font-weight:bold; color:#44aa55;">+{up_rate}%</div></div></div>
        ...
        <div class="right">
            ...
            <div class="fukidashi"><div style="color:#ff4f93; font-weight:bold; font-size:24px; margin-bottom:5px;">🌸 一果のひとこと</div>{comment}</div>
            ...
            <div class="motor-box"><div class="motor-title">⚙️ 一果の機力チェック</div><div style="font-weight:bold; font-size:18px;">{motor_eval}</div></div>
        </div>
    </div>
    <div class="footer"><img src="{footer_img_src}" class="footer-img"></div>
</div>
<div style="text-align:center; padding:20px;">
    <button class="download-btn" onclick="saveImage('.wrapper', 'ikka_newspaper_zenjitsu.png')">画像を保存する</button>
</div>
</body></html>
"""
...
<body><div class="wrapper-live"><div class="telop">🌸 展示終了！一果の最終決定 🌸</div>{header_part}
<div class="main">
    <div class="left"><div class="mainbox-live"><div style="font-size:32px; font-weight:bold; color:#ff4f93;">展示評価：{tenji_rank}</div><div style="font-size:24px;">タイム：{tenji_time} / 進入：{shinnyu}</div><div style="font-size:36px; font-weight:bold; color:#ff4f93; margin-top:15px;">一果判定：{ikka_hantei}</div></div><div class="mainbox-live"><div style="font-size:28px; font-weight:bold;">🎯 的中期待度</div><div style="font-size:72px; font-weight:bold; color:#ff4f93;">{hit_rate}%</div></div><div class="mainbox-live"><div class="section-title">🌸 一果の買い目</div><div style="font-size:24px; font-weight:bold; color:#ff4f93; text-align:center;">本命：{honmei_kaime}</div><div style="border-top:2px dashed #ffb3cf; margin:10px 0;"></div><div style="font-size:20px; text-align:center; color:#666;">押さえ：<br>{osae_kaime.replace('\\n', '<br>')}</div></div></div>
    <div class="right"><img class="character-img" src="{character_src}"><div class="notice-live"><div class="notice-title">📍 直前チェック</div><div class="notice-item">・急上昇：<span class="notice-value">{up_boat}</span></div><div class="notice-item">・危険艇：<span class="notice-value">{danger_boat}</span></div></div><div class="fukidashi" style="margin-top:20px;">{jikkan_comment}</div></div>
</div><div style="background:#ff4f93; color:white; text-align:center; padding:20px; font-size:24px; font-weight:bold;">🌸 一果の最終判断公開中 🌸</div></div>
<div style="text-align:center; padding:20px;">
    <button class="download-btn" onclick="saveImage('.wrapper-live', 'ikka_newspaper_chokuzen.png')">画像を保存する</button>
</div>
</body></html>
"""
