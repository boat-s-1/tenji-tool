...
with input_tab:
    ...
    motor_eval = st.text_area("機力チェック", "1号艇は出足型、3号艇の伸びが節イチ級！", height=100)
    ...
    jikkan_comment = st.text_area("直前コメント", "展示は1号艇優勢！")
    ...
    with st.expander("⚡ キイナのLIVEスタンプ設定"):
        # 💡 用意した画像のパスを指定してください
        stamp_images = {
            "なし": None,
    ...
    with st.expander("👗 初音の女子戦・ピックアップ設定"):
    ...
    * {
    ...
    st.header("📱 SNS画像ツール設定")

    with st.expander("🚨 SNS画像 基本情報"):
    ...
    st.header("🏆 グレードレース12R設定")

    with st.expander("📰 12R新聞 共通情報"):
